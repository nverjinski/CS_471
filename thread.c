...
thread_create(const char *name)
{
	...
	thread->exit_sem = sem_create("exitsem", 1);
	thread->status = THREAD_RUNNING;
	
	// If you add things to the thread structure, be sure to initialize
	// them here.
	if (pid_allocate(&(thread->t_pid)) == -1) { // Ran out of PIDs
		kprintf("Unable to allocate pid\n");
	}
	thread_list[(int) thread->t_pid] = thread;
...
thread_exit(void)
{
	...
	sem_destroy(curthread->exit_sem);
	curthread->status = THREAD_EXITED;
	thread_list[(int) curthread->t_pid] = NULL;
	pid_deallocate(curthread->t_pid);
...
int pid_allocate(pid_t *retval){
	if (pid_sem == NULL) {
		kprintf("Creating semaphore pid_sem\n");
		pid_sem  = sem_create("pid_sem", 1);
	}
	P(pid_sem);
	int i;
	if (first_pid_alloc == 0) { // we need to zero out pid_list
		kprintf("Zeroing out pid_list\n");
		for(i = 0; i < PID_MAX; i++) {
			pid_list[i] = 0;
		}
		first_pid_alloc = 1;
...
int pid_deallocate(pid_t t_pid){
	if((int)t_pid == 0 || (int)t_pid < 0 || (int)t_pid > PID_MAX){
		return EINVAL;
	} else{
		P(pid_sem);
		pid_list[(int)t_pid - 1] = 0;
		V(pid_sem);
	}
	return 0;
}
